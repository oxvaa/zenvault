...
if pkg.get('version')!='5.1.0': errors.append('package.json version is not 5.1.0')
if app.get('expo',{}).get('version')!='5.1.0': errors.append('app.json version is not 5.1.0')
...
payload_path=ROOT/'snack-payload-5.1.0.json'
if payload_path.exists():
    ...
    if payload.get('manifest',{}).get('name')!='Zenvault Bank & Co. 5.1.0': errors.append('Snack payload manifest has wrong name/version')
...
if index_path.exists() and 'snack-payload-5.1.0.json' not in index_path.read_text(): errors.append('index.html does not target the 5.1.0 Snack payload')

required=['App.js','AuthScreen.js','Zenvault3.js','Zenvault5.js','Home5.js','SmartTransfer.js','engine.js','CardArtwork.js','Membership.js','ThemeCatalog.js','AdminStudio6.js','AdminReasons.js','index.html','RELEASE-NOTES-5.1.0.md','snack-payload-5.1.0.json']
