...
export default function ThemeStudio({selected,dark,u,t,onSelect,onRewards,onUnlimited}){
 ...
 const themes=useMemo(()=>APP_THEMES.filter(m=>(group==='Vše'||(group==='Nové'?['new','z5'].includes(m.edition):m.group===group))&&m.name.toLowerCase().includes(query.trim().toLowerCase())),[group,query]);
 const label={color:t.text},muted={color:t.muted};
 return <View>
  <Text style={[muted,{fontSize:10,fontWeight:'700',letterSpacing:2,marginBottom:8}]}>THEME STUDIO · 36</Text>
  <Text style={[label,{fontWeight:'800',fontSize:29,letterSpacing:-1}]}>Změň barvy. Zůstaň svůj.</Text>
  <Text style={[muted,{lineHeight:21,fontSize:13,marginTop:8,marginBottom:18}]}>Zenvault 5 přidává Bright Red, Bright Red & Black a Bright Red & Silver. K tomu zůstává celé původní Theme Studio a exkluzivní Unlimited Monochrome.</Text>
  <MiniPreview theme={selected} dark={previewDark} hero/>
  <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginVertical:14}}><View style={{flex:1}}><Text style={[label,{fontWeight:'700',fontSize:16}]}>{selected.name}</Text><Text style={[muted,{fontSize:11,marginTop:4}]}>{selected.forceLight?'Aktivní · vždy světlý':selected.forceDark?'Aktivní · vždy tmavý':'Aktivní motiv'}</Text></View><View style={{flexDirection:'row',gap:5}}>{[false,true].map(v=><Pressable key={String(v)} accessibilityRole="button" accessibilityLabel={v?'Tmavý náhled motivů':'Světlý náhled motivů'} accessibilityState={{selected:previewDark===v}} onPress={()=>setPreviewDark(v)} style={{padding:12,borderRadius:24,backgroundColor:previewDark===v?t.button:t.soft}}><Ionicons name={v?'moon-outline':'sunny-outline'} size={18} color={previewDark===v?'#FFF':t.accent}/></Pressable>)}</View></View>
